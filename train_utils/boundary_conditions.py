...
def bc_loss(model_y,y,bc_index,derivatives, loss_function):
    ...
    d_loss = []
    for patch in boundary_mapping[globals()["ARGS"].name]['D_BC']['u']:
        d_loss += [loss_function(model_y[:,bc_index[patch],0],y[:,bc_index[patch],0])]
    for patch in boundary_mapping[globals()["ARGS"].name]['D_BC']['v']:
        d_loss += [loss_function(model_y[:,bc_index[patch],1],y[:,bc_index[patch],1])]
    for patch in boundary_mapping[globals()["ARGS"].name]['D_BC']['p']:
